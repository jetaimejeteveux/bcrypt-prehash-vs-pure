...
public class SHA512 {
    ...
    public static String hash(byte[] input) {
        ...
        input = Formula.pad(input);

        // Break the padded input up into blocks
        long[][] blocks = Formula.toBlocks(input);

        // And get the expanded message blocks
        long[][] W = Formula.Message(blocks);
        ...
        for (int i = 0; i < blocks.length; i++) {
            ...
            for (int j = 0; j < 80; j++) {
                long t1 = h + Formula.Sigma1(e) + Formula.Ch(e, f, g) + Constants.K[j] + W[i][j];
                long t2 = Formula.Sigma0(a) + Formula.Maj(a, b, c);
                h = g;
                g = f;
                f = e;
                e = d + t1;
    ...
    public static byte[] hashToByte(byte[] input) {
        // First pad the input to the correct length, adding the bits specified in the SHA algorithm
        input = Formula.pad(input);
        ...
        // After everything is done, return  hash as a byte
        byte[] finalHashValue = new byte[64];

        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                finalHashValue[i * 8 + j] = (byte) (buffer[i] >>> (56 - j * 8));
            }
        }
